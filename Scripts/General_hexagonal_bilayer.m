...
(* ::Input:: *)
(*General  hexagonal Bilayer Moire  pattern under strain*)


(* ::Input:: *)
(*Definiton of hexagonal Bravais lattice *)


(* ::Input:: *)
(*a:=0.329(*Change to the wanted lattice constant*)*)
(*unitvectorW[x_,y_]:=a*{x,y}*)
(*unitvectorSe[x_,y_]:=a*{x,y+2/3 Sin[120 Degree]}*)
(*unitCell[x_,y_]:={Red,Disk[unitvectorW[x,y],a*0.1],Blue,Disk[unitvectorSe[x,y],a*0.1],Gray,*)
...
(* ::Input:: *)
(*Remove[ec,es,size,theta,theta0,delta,mu,phis]*)
(*nu:=0.19(*Adjust Poisson ratio here: Preset to the WSe2 value https://aip.scitation.org/doi/pdf/10.1063/1.4774090*)*)
(*phis:=90*)
(*ec[eu_]:=0.5*(1-nu)*eu*)
(*es[eu_]:=0.5*(1+nu)*eu*)
(*Manipulate[With[{points=Block[{unitVectA={Cos[120 Degree],Sin[120 Degree]},unitVectB={1,0}},Table[unitCell@@(unitVectA j+unitVectB k),{j,-size,size},{k,-size+Ceiling[j],size+Ceiling[j]}]]},Column[{Graphics[{GeometricTransformation[points,strainmatrix[ec[eu],es[eu],1,phis].rotationmatrix[psi[theta0][[1]]]],GeometricTransformation[points,(1+delta)*strainmatrix[ec[eu],es[eu],mu,phis].rotationmatrix[psi[theta0][[1]]+theta]],FaceForm[None],EdgeForm[Red],Parallelogram[{0,0},{-moirevector1[ec[eu],es[eu],delta,theta,mu,phis,theta0],-moirevector2[ec[eu],es[eu],delta,theta,mu,phis,theta0]}],Parallelogram[{0,0},{moirevector1[ec[eu],es[eu],delta,theta,mu,phis,theta0],-moirevector2[ec[eu],es[eu],delta,theta,mu,phis,theta0]}],Parallelogram[{0,0},{-moirevector1[ec[eu],es[eu],delta,theta,mu,phis,theta0],moirevector2[ec[eu],es[eu],delta,theta,mu,phis,theta0]}],Parallelogram[{0,0},{moirevector1[ec[eu],es[eu],delta,theta,mu,phis,theta0],moirevector2[ec[eu],es[eu],delta,theta,mu,phis,theta0]}]},PlotRange->{{-1.66*a*size,1.66*a*size},{-1*a*size,1*a*size}},ImageSize-> Large]}]],{{eu,0,"Uniaxial, \!\(\*SubscriptBox[\(\[Epsilon]\), \(u\)]\)"},-0.1,0.1,0.01},{{delta,0.0,"Lattice mismatch \[Delta]"},0,0.5},{{theta,1,"Twist angle \[Theta]"},0,5.1},{{mu,0,"Strain transfer \[Mu]"},0,1},{{phis,0,"Shear Angle \!\(\*SubscriptBox[\(\[Phi]\), \(s\)]\)"},0,90},{{theta0,0,"Lattice Orientation \!\(\*SubscriptBox[\(\[Theta]\), \(0\)]\)"},0,360},{{size,5,"Amount of unit cells shown"},1,50}]*)
